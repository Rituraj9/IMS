...
from recruiters.models import Job, Applicants, Selected, Shortlisted, first_Selected, second_Selected
...
def job_search_list(request):
    ...
    if(query == None):
        ...
        internship_list = Job.objects.filter(
            internship__icontains=query).order_by('-date_posted')
        for i in title_list:
...
def job_detail(request, slug):
    ...
    jobs2 = Job.objects.filter(
        internship__icontains=job.internship).order_by('-date_posted')
...
def applied_jobs(request):
    ...
    statuses = []
    for job in jobs:
        if Applicants.objects.filter(job=job.job).filter(applicant=request.user).exists():
            statuses.append(0)
        elif Shortlisted.objects.filter(job=job.job).filter(applicant=request.user).exists():
            statuses.append(1)
        elif first_Selected.objects.filter(job=job.job).filter(applicant=request.user).exists():
            statuses.append(2)
        elif second_Selected.objects.filter(job=job.job).filter(applicant=request.user).exists():
            statuses.append(3)
        elif Selected.objects.filter(job=job.job).filter(applicant=request.user).exists():
            statuses.append(4)        
...
def intelligent_search(request):
    ...
    if profile:
        jobs = Job.objects.filter(
            internship=profile.looking_for).order_by('-date_posted')
    else:
...
def download(request, path):
    file_path = os.path.join(settings.MEDIT_ROOT, path)
