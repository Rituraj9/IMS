...
from .models import Job, Applicants, Shortlisted, first_Selected, second_Selected, Selected
...
def rec_details(request):
    ...
    return render(request, 'recruiters/hiring.html', context)
...
def job_candidate_search(request, slug):
    ...
    relevant_candidates = []
    common = []
    applicants = Profile.objects.filter(looking_for=job.internship)
    job_skills = []
...
@login_required
# for first round select
def first_round_select_applicant(request, can_id, job_id):
    job = get_object_or_404(Job, slug=job_id)
    profile = get_object_or_404(Profile, slug=can_id)
    user = profile.user
    selected, created = Shortlisted.objects.get_or_create(job=job, applicant=user)
    applicant = Applicants.objects.filter(job=job, applicant=user).first()
    applicant.delete()
    return HttpResponseRedirect('/hiring/job/{}/shortlisted'.format(job.slug))

@login_required
# for 2nd round
def second_round_select_applicant(request, can_id, job_id):
    job = get_object_or_404(Job, slug=job_id)
    profile = get_object_or_404(Profile, slug=can_id)
    user = profile.user
    selected, created = first_Selected.objects.get_or_create(job=job, applicant=user)
    applicant = Shortlisted.objects.filter(job=job, applicant=user).first()
    applicant.delete()
    return HttpResponseRedirect('/hiring/job/{}/first'.format(job.slug))

@login_required
# for 3rd round
def third_round_select_applicant(request, can_id, job_id):
    job = get_object_or_404(Job, slug=job_id)
    profile = get_object_or_404(Profile, slug=can_id)
    user = profile.user
    selected, created = second_Selected.objects.get_or_create(job=job, applicant=user)
    applicant = first_Selected.objects.filter(job=job, applicant=user).first()
    applicant.delete()
    return HttpResponseRedirect('/hiring/job/{}/second'.format(job.slug))    

@login_required
# for final selected list
def select_applicant(request, can_id, job_id):
    job = get_object_or_404(Job, slug=job_id)
    profile = get_object_or_404(Profile, slug=can_id)
    user = profile.user
    selected, created = Selected.objects.get_or_create(job=job, applicant=user)
    applicant = second_Selected.objects.filter(job=job, applicant=user).first()
    applicant.delete()
    return HttpResponseRedirect('/hiring/job/{}/selected'.format(job.slug))

@login_required
def shortlisted_list(request, slug):
    job = get_object_or_404(Job, slug=slug)
    selected = Shortlisted.objects.filter(job=job).order_by('date_posted')
    profiles = []
    ...
    return render(request, 'recruiters/first_round_select.html', context)

def first_selected_list(request, slug):
    job = get_object_or_404(Job, slug=slug)
    selected = first_Selected.objects.filter(job=job).order_by('date_posted')
    profiles = []
    for applicant in selected:
        profile = Profile.objects.filter(user=applicant.applicant).first()
        profiles.append(profile)
    ...
    return render(request, 'recruiters/second_round_select.html', context)

def second_selected_list(request, slug):
    job = get_object_or_404(Job, slug=slug)
    selected = second_Selected.objects.filter(job=job).order_by('date_posted')
    profiles = []
    for applicant in selected:
        profile = Profile.objects.filter(user=applicant.applicant).first()
        profiles.append(profile)
    ...
    return render(request, 'recruiters/third_round_select.html', context)
